...
pub fn render(cfg: &Config) -> String {
    ...
    out.push_str(&format!("    ct state new log prefix \"{LOG_PREFIX_IN}\" queue num {QUEUE_IN}\n"));
    out.push_str("  }\n");
    ...
    out.push_str(&format!("    ct state new log prefix \"{LOG_PREFIX_OUT}\" queue num {QUEUE_OUT}\n"));
    out.push_str("  }\n");
    out.push_str("}\n");
    out
}
...
mod tests {
    ...
    fn rule(action: Action, proto: Proto, src: &str, port: Option<u16>) -> Rule {
        Rule { id: 1, action, proto, src: src.into(), port, enabled: true }
    }

    #[test]
    fn allow_with_src_and_port() {
        let cfg = Config { rule: vec![rule(Action::Allow, Proto::Tcp, "192.168.1.0/24", Some(22))], ..Config::default() };
        let out = render(&cfg);
    ...
    fn deny_any_any_drops_everything() {
        let cfg = Config { rule: vec![rule(Action::Deny, Proto::Any, "any", None)], ..Config::default() };
        let out = render(&cfg);
    ...
    fn disabled_rule_is_skipped() {
        ...
        r.enabled = false;
        let cfg = Config { rule: vec![r], ..Config::default() };
        let out = render(&cfg);
    ...
    fn ipv6_src_uses_ip6_saddr() {
        let cfg = Config { rule: vec![rule(Action::Allow, Proto::Tcp, "fd00::1/64", Some(22))], ..Config::default() };
        let out = render(&cfg);
    ...
    fn queue_line_present_in_both_chains_without_bypass() {
        ...
        let cfg = Config { rule: vec![], ..Config::default() };
        let out = render(&cfg);
    ...
    fn queue_line_comes_after_ip_rules_in_input_chain() {
        let cfg = Config { rule: vec![rule(Action::Deny, Proto::Tcp, "1.2.3.4", None)], ..Config::default() };
        let out = render(&cfg);
        let input_chain = out.split("chain output").next().unwrap();
        let rule_pos = input_chain.find("1.2.3.4 tcp drop").unwrap();
        let queue_pos = input_chain.find("queue num").unwrap();
        assert!(rule_pos < queue_pos, "ip rule must be evaluated before the queue fallback");
    }

    #[test]
    fn port_without_explicit_proto_defaults_tcp() {
        let cfg = Config { rule: vec![rule(Action::Allow, Proto::Any, "any", Some(443))], ..Config::default() };
        let out = render(&cfg);
